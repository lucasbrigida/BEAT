 ...
 * -------------------------------------------
 */
var secretCode = ['top-left', 'top-left', 'bottom-left', 'bottom-left'];
var commands = [];

function setTouchCode(cb) {
    return function(e) {
        e.preventDefault();
        var elem = e.target;
        var cmd = elem.getAttribute('data-direction');   
        console.log(cmd);

        if (commands.length < secretCode.length - 1) return commands.push(cmd);
        for (var i in commands) {
            if (commands[i] !== secretCode[i]) return commands = [];
        }
